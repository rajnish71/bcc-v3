**# Bug fixes, Feature Requests and Feedback Sessions**
...
~~0.1 following open items :~~



~~Item, Action needed, When~~

~~Card, 55×87mm portrait 2-sided, Dedicated Module 02 revisit session After Module 06~~



~~Google callback URL, Dedicated auth-fix session (check Console + env var), After Module 06~~





~~0.2 password reset option / forgot password userid~~
...
~~1. logged in user should not see sign in button on header or anywhere:~~

~~2. logged in member should not see Become a member button on header or anywhere, only on membership management page he should see if upgrade path available to him / her~~

~~3. for logged in user / member logout option should be easily available~~

~~4. user should be able to login with their userid also not just email~~

~~5. also check if other users also password is argon compatible, other wise here is argon hash for 'Bcc2026!' ...... make it default for all current users, new users will be asked to create their own passwords once gmail / facebook login starts working~~

~~6. Lets import Journal pages from legacy site with images, it was originally migrated from our wordpress site, infact we have to check archieved history of our sitemap and do proper 301 for these pages. Also new journal creation to be created using some good rtf editor~~

7\. ~~from the open items :~~  7.65

&#x09;~~i. Card 55×87mm portrait 2-sidedDedicated Module 02 revisit sessionAfter Module 06 ( Item, Action needed, When / Card, 55×87mm portrait 2-sided, Dedicated Module 02 revisit session After Module 06 )~~

&#x09;~~ii. Google login - callback URLDedicated auth-fix session (check Console + env var)After Module 06 - when trying to register using google, user is getting created but page is stuck on a blank page and same while trying to login through 		gmail ( Item, Action needed, When / Google callback URL, Dedicated auth-fix session (check Console + env var), After Module 06 )~~



&#x09;~~iii. Facebook login step by step~~

~~8. My profile page https://v3bcc.bhopal.info/gallery/photographer/rajnishkhare/ is giving 404~~

~~9. https://v3bcc.bhopal.info/showcase/ is not showing photos in a grid as planned in wireframe but stacked in single column full width. Please refer to our legacy showcase page https://bcc.bhopal.info/showcase , only major difference we want is photo should not be cropped or recomposed in grid, should be shown in same aspect ratio as original. Also looks like images on showcase pages have not inherited their exif , meta and genre from legacy site~~

~~10 home page - no hero image~~

~~11 home page - no photo grid on section Our members on the wall~~

~~12. home page - no upcoming activities~~

~~13. home page - no article from journal~~

~~14. Admin Dashboard / conslole to manage users same as 3.21~~ Added to Roadmap as V6 22 Phase E

~~15 password reset option / forgot password userid~~



Stage 3 : many still Pending

~~16. photographers profile - bio and other profile details not migrated -  same as 5.36~~ 7.50

~~17. Home Page - Members wall section is showing recently uploaded photos instead of grid of photos, max 1 photo per member - done~~

~~18. Home Page - Hero Spotlight should be admin selectable from admin console. Spotlight, Photoseriec and Contest images on home page has to be panorama, so we need a system to identify them. other wise system will change their aspect ratio and crop - admin pages to be built - pending~~ 7.63

~~19. creation of stories by photographers from hub portfolio page - pending~~ 7.62

~~21. Admin page to be build separate from Members hub - acutally Members hub can have multiple tabs, Members Hub / Admin Page / Moderator / Author - pending - same as 2.14~~ Added to Roadmap as V6 22 Phase E

~~22. Data cleaning - User ID and Member ID sequential without immediately after we allot permanent numbers to members. after that no hard delete / edit ever - pending~~ 7.61
...
~~23. Joining Date in profile edit~~

24\. ~~temp membership number to all members ( except founding 01 to 07)~~ - ~~should be visible in dummy card in the profile page~~ 7.45

~~25. Profile Cover aspect ratio should be changed, make it slightly more broader, its very thin as of now. mention cover banner aspect ratio / pixel size below the placeholder~~

~~26. To verify cover pic size for different medium and find a safe zone that is visible on all devices~~ 7.46
...
27\. ~~Home PAge - Upcoming activity cards not visible, we had few dummy activites in legacy site, was that not imported? We need to create some dummy activities and images to show in this place.~~

~~28. Similarly Section "One Community. Four Ways To Participate." should have some dummy photos, but relevant and data to show. F 5.2.2. Populated "One Community. Four Ways to Participate." — 4 club cards with real copy and distinct dark gradients --> ok for now but should be better formatted~~ 7.60

~~29. For section , use this photo by Kshitij Patle as background image in the image placeholder till we wire this section. Give a suitable heading like "Serene Morning around Tajul Masajid" - Photo - "E:\\WebProjects\\BCC-V3\\frontend\\public\\images\\hero.jpg"~~

~~30. Section - "Membership three Steps" - change the description , replace it with something more relevant. current "Three-step educational flow communicating the membership journey. Registration, Application, and Membership Active are presented as distinct milestones. Do not collapse into a single CTA.". Similarly under Become a member - Bhopal Camera Club, change following text "Photography community, mentorship, and exhibitions. Three clear steps to membership". replace with something meaning full/~~  7.57

31\. ~~Same in journal -replace "Three JournalCard components in standard layout. Final editorial section before the footer. All Stories → links to /journal." with something better~~

~~32. Activity Page - Upcoming activity - no photo card in list view or grid~~ 7.58

~~33. in hub home when profile edits , sidebar should not scroll , is this possible. Besides Hub Profile page looks very raw, needs to be refined, why everything is scrolling down, it was not designed this way in wireframe, there is tab for everything, each tab should show information about itself only~~ 7.59

~~34. To maximize the SEO value every photo should have alt text, the alt text for each image includes the photographer's name and the subject, like this:~~

~~alt="Indian Roller bird captured by Bhopal Camera Club member \[Name]" or alt="Taj Mahal reflection by Bhopal Camera Club member \[Name]". This helps the images rank in Google Images under local photography searches.~~

~~35.  V6 05 - Photographers profile page should also have a justified grid as in home page and showcase - present grid is not looking good~~

~~36. In Photographers Profile PAge following have not migrated - Display Photo, Biography, ............actually almost 90 per cent of data is missing, no address, blood group of any any one, in case of emergency, camera gears, awards and recognition, distintions ...............everything is missing same as~~ 3.16 -----> 7.50



Stage 6

~~37. Drop Down navigation menu, when we are taking mouse lower, its disappearing - photographers/ showcase in showcase, we cant click because of this, same is happening with activites drop down~~

~~38. home page journal - images should not stick to one another, all title sub title and text should align with each other~~ 7.49

~~39. IG YT FB X LI on members profile, instead of text can we have icon of those social media displayed~~

~~40. light box is not showing all photos of sauvik acharyya. Few of the photos of Sauvik Acharyya are not visible on light box, one of them is 'The Story Tree'~~

~~41. Favicon visibility - "E:\\WebProjects\\BCC-V3\\frontend\\public\\favicon.ico" please make it favicon~~

~~42.  profile social media and about / bio is not saving for basic members, as a admin my details are getting saved~~ 7.56

~~43. Profile picture is visible on member hub but not on public view~~

~~44. In Page showcase : https://bcc.bhopal.info/showcase/ - second section "The Photographers Meet the members behind the wall." is not displaying properly  . As per wireframe it has to show random 8 or 12 users grid - Grid of contributing photographer profile cards below the gallery. Avatar, display name, short bio, photo count. Each card links to /photographers/\[username]. . it is only showing text. do the needful~~
...
~~5P 1. Removed all spec/placeholder text from Photo Series, Membership, and Journal sections --> we were not suppose to remove them but rewrite them to make it look more relevant~~ 7.57



~~5P 2. Restored Activity row thumbnails — backend now exposes banner\_url via ikUrl() in EventSummary; frontend renders image or falls back to gradient --> I will have to check this~~ 7.58



~~F5.2 — Photographer Profile Data Recovery -~~ 7.50



~~Full field-by-field audit produced: ProjectDocs/SessionSummaries/2026-07-12 - Photographer Profile Data Audit.md~~

~~Recovered user\_awards → added to API response + About tab (Awards \& Distinctions card)~~

~~Recovered user\_photo\_titles → added to API response + About tab (Photography Society Titles card)~~

~~Recovered users.awards\_html → exposed in API~~

~~Confirmed private fields (address, blood group, emergency contact) correctly excluded from public profile~~
...
~~do not display these publicly but make it visible and editable on hub / profile and on i cards whatever is applicable~~
...
~~F5.3 — Photographer Profile Gallery~~



~~5P 3.1. Replaced 3-column CSS grid with canonical justified layout (buildProfileRows() using flush(true) to include partial last row so all photos always appear)~~



~~F5.4 — SEO Alt-Text --> good but black band in the bottom is misaligned and Load More Photographers link onit is not working - blank band is still misalligned~~ 7.55
...
~~6P 1	Photographer profile 404 fix — Added \_profile shell to getStaticPaths() so Astro builds a generic fallback page; Nginx try\_files serves it for any unknown username without requiring a rebuild	--> not sure will have to check this workflow. This will ensure when a new member registers and once approved his page is automatically created without requiring a npm build ( last resort cron job running npm build every few hours )~~ 7.54



~~6P 2	Lightbox: missing photos + redesign — Fixed gallery fetch to pass Bearer token (so MEMBERS\_ONLY photos load); redesigned lightbox to 2-column (left: image + prev/next arrows, right: metadata rail); added touch swipe for mobile	✅ Done --> light box need further refinement. missing photos of Sauvik still not showing though thumbnail of those photos are shown in gallery grid. EXIF data missing. --> still needs lot of refinement and formatting proper allingment~~ 7.53
...
~~6P 3	Journal cards sticking together — Added gap: 20px to .journal-grid, changed to transparent background, gave each .journal-card its own individual border -->  ok visible we should think of aligning text als~~o	7.49



~~6P 4	Admin console — Built end-to-end: enhanced pending endpoint with user info join, new admin password reset endpoint, new member search endpoint, and /hub/admin/ frontend page with approve/reject workflow + password reset UI - admin console reachable by manually entering url , no link on hub sidebar or anywhere in nav. Clicking on Admin Console on top nav goes to member hub	✅ Done~~
...
~~6P 5. Dr Anil Bhati "Twilight Hours, Maldives" for spotlight hero	Waiting on ImageKit URL -  from you we use store it in Cloudflare R2  uploads/1781579369859-2387d206.jpg our endpoint configured on imagekit for cloudflare folder is https://ik.imagekit.io/duynda7oq/~~



~~This tells us that we should audit entire upload workflow and save it somewhere , so that in future we remember what Cloudflare is doing and what Imagekit is gonna do~~ 7.52



~~6P 6. Sauvik photos with missing R2 originals	Need re-upload; lightbox degrades to 800px medium for now but its visible now on lightbox, though i will ask Sauvik to re upload~~ 7.51 ( manually ask Sauvik to do this )



~~6P 7. Hub profile: private fields (address, blood group, emergency contact) visible internally	Not addressed this session --> This should be top priority next session~~ 7.50



~~6P 8. Journal card text alignment	Noted but not changed (minor, user said "think about it") - do it now~~  7.49
