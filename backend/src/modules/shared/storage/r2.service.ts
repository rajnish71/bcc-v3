...
// R2_SECRET_ACCESS_KEY, R2_BUCKET). The client initialises LAZILY and
  ...
  private ensureClient(): S3Client {
    ...
    if (!endpoint || !accessKeyId || !secretAccessKey || !bucket) {
      throw new ServiceUnavailableException(
        'R2 storage is not configured. Set R2_ENDPOINT, R2_ACCESS_KEY_ID, R2_SECRET_ACCESS_KEY and R2_BUCKET in backend/.env (values must be quoted if they contain special characters).',
      );
    }
