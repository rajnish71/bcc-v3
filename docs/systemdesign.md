# Engineering Mirror of V6 99 Design Constitution

**Source of Truth:** [V6 99 System Design Constitution](file:///ProjectDocs/Wireframes/V6/V6%2099%20System%20Design%20Constitution/V6%2099%20System%20Design%20Constitution.md)

*This document is an implementation reference. Do not edit independently.*

---

## Engineering Implementation & Migration Policy

> [!IMPORTANT]
> - **V6 Design Tokens** are the official and canonical design token system of the BCC Unified Platform.
> - **Backward Compatibility Aliases** defined in `tokens.css` are strictly temporary.
> - **New Development MUST NOT** use compatibility aliases in any shape or form.
> - Legacy compatibility aliases exist solely to facilitate an incremental, safe migration without breaking existing V3 code.

### 1. Incremental Migration Policy
This repository does **not** employ a repository-wide search-and-replace for design tokens, as this carries high regression risks. Instead, migration is strictly incremental:
- Whenever any existing page, template, or stylesheet is modified for a bug fix, feature addition, or layout enhancement, the engineer **must** replace all deprecated token usages on that page.
- Fully migrate that file to use the canonical V6 tokens.
- Remove all references to the legacy aliases in the modified file.

### 2. Future Development Rule
Any newly created:
- Page (`.astro`)
- Layout (`.astro`)
- Component (`.astro`)
...
**must exclusively** use canonical V6 design tokens. The use of compatibility aliases in any new code is strictly prohibited.
